...
```
### Source Code Notes
**1.  nested for loop: 한줄에 multiple variable 선언하기?**
...
**알고리즘상의 차이는?**
i의 for loop을 한번 돌때 전자는 두번째 for loop에서 j= 1, 2, ... ballArray[j].s < ballArray[i].s 번 돌고

후자는 이전 j값이 5였다면 두번째 for loop에서 j = 6으로 시작하므로 시간 복잡도가 절약된다.



**2. ```ballArray[j].s < ballArray[i].s``` VS ```j < i```**
